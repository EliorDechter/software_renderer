...
internal gen_entity *
AddEntity(world_generator *Gen, gen_create_entity *Creator)
{
    gen_entity *Result = PushStruct(&Gen->Memory, gen_entity);
    Result->Creator = Creator;
    return(Result);
}

internal gen_entity_tag *
AddTag(world_generator *Gen, gen_entity *Entity, asset_tag_id ID, f32 Value)
{
    Assert(Entity->TagCount < ArrayCount(Entity->Tags));
    
    gen_entity_tag *Tag = Entity->Tags + Entity->TagCount++;
    Tag->ID = ID;
    Tag->Value = Value;
...
internal gen_entity_group *
AddEntityGroup(world_generator *Gen, gen_room *Room)
{
    gen_entity_group *Group = PushStruct(&Gen->Memory, gen_entity_group);
    Group->Next = Room->FirstEntityGroup;
    Room->FirstEntityGroup = Group;
    
    return(Group);
}

internal void
PlaceEntity(world_generator *Gen, gen_entity *Entity, gen_entity_group *Group)
{
    Assert(Entity);
    Entity->Next = Group->FirstEntity;
    Group->FirstEntity = Entity;
}

internal void
PlaceEntity(world_generator *Gen, gen_entity *Entity, gen_room *Room)
{
    Assert(Entity);
    gen_entity_group *Group = AddEntityGroup(Gen, Room);
    PlaceEntity(Gen, Entity, Group);
    Assert(Group->FirstEntity);
}

internal void
AppendEntity(world_generator *Gen, gen_entity *Parent, u32 DirMask, gen_entity *Child)
{
    Assert(!Child->Next);
    Child->Next = Parent->Next;
    Parent->Next = Child;
    
    Parent->AllowedDirectionsForNext = DirMask;
}

internal gen_entity *
PlaceCat(world_generator *Gen)
{
    gen_entity *Result = 0;
    
    for(gen_option_iterator Iter = IterateOptions(Gen, GenOption_Cat);
        IsValid(&Iter);
        Advance(&Iter))
    {
        ...
        if(1)
        {
            Result = AddEntity(Gen, AddCat);
            AddTag(Gen, Result, Tag_Cat, 1.0f);
...
internal gen_entity *
PlaceOrphan(world_generator *Gen, asset_tag_id OrphanNameTag)
{
    gen_entity *Result = 0;
        ...
        Advance(&Iter))
    {
        ...
        if(1)
        {
            Result = AddEntity(Gen, AddOrphan);
            AddTag(Gen, Result, Tag_Orphan, 1.0f);
            AddTag(Gen, Result, Tag_FacingDirection, 0.75f*Tau32);
            PlaceEntity(Gen, Result, Room);
            
            gen_entity *Child = AddEntity(Gen, AddConversation);
            AppendEntity(Gen, Result, BoxMask_South, Child);
    ...
    if(Result)
    {
        AddTag(Gen, Result, OrphanNameTag, 1.0f);
    }
...
internal gen_entity *
PlaceSnake(world_generator *Gen, gen_room *Room)
{
    gen_entity *Result = AddEntity(Gen, AddSnake);
    AddTag(Gen, Result, Tag_Bones, 1.0f);
    PlaceEntity(Gen, Result, Room);
...
CreateOrphanage(world_generator *Gen)
{
    ...
    if(1)
    {
        gen_entity *Scenery = AddEntity(Gen, AddObstacle);
        AddTag(Gen, Scenery, Tag_Chair, 1.0f);
        AddTag(Gen, Scenery, Tag_FacingDirection, 0.0f*Tau32);
        PlaceEntity(Gen, Scenery, MainRoom);
    }
...
CreateWorld(world *World, game_assets *Assets)
{
    ...
    gen_entity *Hannah = PlaceCat(Gen);
    AddTag(Gen, Hannah, Tag_Ghost, 1.0f);
    AddTag(Gen, Hannah, Tag_Birman, 1.0f);
    
    gen_entity *Fred = PlaceCat(Gen);
    AddTag(Gen, Fred, Tag_Brown, 1.0f);
    AddTag(Gen, Fred, Tag_Tabby, 1.0f);
    
    gen_entity *Molly = PlaceCat(Gen);
    AddTag(Gen, Molly, Tag_Gray, 1.0f);
    AddTag(Gen, Molly, Tag_Tabby, 1.0f);
    
    PlaceOrphan(Gen, Tag_Baby);
    PlaceOrphan(Gen, Tag_Brahm);
    PlaceOrphan(Gen, Tag_Carla);
    PlaceOrphan(Gen, Tag_Cassidy);
    PlaceOrphan(Gen, Tag_Drew);
    PlaceOrphan(Gen, Tag_Dylan);
    PlaceOrphan(Gen, Tag_Giles);
    PlaceOrphan(Gen, Tag_Kline);
    PlaceOrphan(Gen, Tag_Laird);
    PlaceOrphan(Gen, Tag_Lambert);
    PlaceOrphan(Gen, Tag_Rhoda);
    PlaceOrphan(Gen, Tag_Slade);
    PlaceOrphan(Gen, Tag_Sunny);
    PlaceOrphan(Gen, Tag_Viva);
