...
PushOccluder(render_group *Group, v3 P, v3 Radius, v4 Color)
{
    ...
    if(Group->LightingEnabled)
    {
        ...
        Box->BoxMax = P + Radius;
        Box->RefC = Color.rgb;
        Box->Pack.FirstChildIndex = 0;
        Box->Pack.IsLight_IsLeafContainer = Box->Pack.ChildCount = 0;
    }
}
...
PushLight(render_group *Group, v3 P, v3 Radius, v3 Emission)
{
    ...
    if(Group->LightingEnabled)
    {
        ...
        Box->RefC = Emission;
        Box->Pack.FirstChildIndex = 0;
        Box->Pack.ChildCount = 0;
        Box->Pack.IsLight_IsLeafContainer = LightBox_IsLight;
    }
}
...
{
    render_setup NewSetup = RenderGroup->LastSetup;
    NewSetup.VoxelMinCorner = VoxelMinCorner;
    NewSetup.VoxelInvTotalDim = VoxelInvTotalDim;
    PushSetup(RenderGroup, &NewSetup);
}
...
inline void
EnableLighting(render_group *RenderGroup, rectangle3 LightingBounds)
{
    RenderGroup->LightingEnabled = true;
    RenderGroup->LightBounds = LightingBounds;
}
