...
#define LIGHT_LOOKUP_VOXEL_DIM_Z 16
...
// TODO(casey): Leaf container stuff can go away now, because that's handled
// by the Kd tree.
enum lighting_box_flag
{
    LightBox_IsLeafContainer = 0x1,
    LightBox_IsLight = 0x2,
};

struct lighting_box_pack
{
    // TODO(casey): Do we really want in-line spatial hierarchy like this?
    // It seems like it could just be out-of-band...
    u16 FirstChildIndex;
    u8 ChildCount;
    u8 IsLight_IsLeafContainer;
    
    // TODO(casey): Lights can't be leaf containers and vice versa, so probably just make this a bool again that works for both!
};
CTAssert(sizeof(lighting_box_pack) == sizeof(u32));
