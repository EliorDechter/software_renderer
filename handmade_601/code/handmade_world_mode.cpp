          ...
          brain_id BrainID)
{
    ...
    AddTag(Body, Tag_Orphan, 1.0f);
    AddTag(Body, Tag_Hero, 1.0f);
    AddTag(Body, Tag_Hero, 1.0f);
    
    AddTag(Glove, Tag_Glove, 1.0f);
    AddTag(Glove, Tag_Fingers, 1.0f);
    
    entity_visible_piece *BodyPiece = AddPiece(Body, Asset_Body, HeroSizeC, V3(0, 0, 0), Color, PieceMove_AxesDeform);
    entity_visible_piece *HeadPiece = AddPiece(Body, Asset_Head, HeroSizeC, V3(0, 0, 0.15f), Color);
    entity_visible_piece *HatPiece = AddPiece(Body, Asset_Item, HeroSizeC, V3(0, 0, 0.25f), Color);
    entity_visible_piece *GlovePiece = AddPiece(Glove, Asset_Hand, HeroSizeC, V3(0, 0, 0), Color);
    ...
    ConnectPiece(Body, HeadPiece, HHAAlign_TopOfHead, HatPiece, HHAAlign_Default);
...
PlayWorld(game_state *GameState)
{
    ...
    InitLighting(&WorldMode->TestLighting, &GameState->ModeArena, WorldMode->FundamentalUnit);
...
internal world_sim
BeginSim(game_assets *Assets, memory_arena *TempArena, world *World, world_position SimCenterP, rectangle3 SimBounds, r32 dt)
{
    world_sim Result = {};
    ...
    sim_region *SimRegion = BeginWorldChange(Assets, TempArena, World, SimCenterP, SimBounds, dt);
...
internal PLATFORM_WORK_QUEUE_CALLBACK(DoWorldSim)
{
    ...
    world_sim WorldSim = BeginSim(Work->GameState->Assets, &Arena, World, Work->SimCenterP, Work->SimBounds, Work->dt);
                     ...
                     editable_hit_test *HitTest)
{
    ...
    f32 FocusMinZ = WorldMode->Camera.ExpectedFocusMinZ;
    f32 FocusMaxZ = WorldMode->Camera.ExpectedFocusMaxZ;
    ...
    f32 FocusZ = 0.5f*(WorldMode->Camera.ExpectedFocusMinZ + WorldMode->Camera.ExpectedFocusMaxZ);
    rectangle3 WorldCameraRect = GetCameraRectangleAtTarget(RenderGroup, FocusZ);
...
#else
    world_position LightingCenter = WorldMode->Camera.SimulationCenter;
#endif
    BeginLightingComputation(&WorldMode->TestLighting, RenderGroup, World,
                             WorldMode->Camera.SimulationCenter, 
                             LightingCenter, WorldMode->UpdatingLighting);
                                  ...
                                  dt);
    {
        ...
        if(CameraEntity)
        {
            UpdateCameraForEntityMovement(&WorldMode->Camera, WorldSim.SimRegion, World, CameraEntity, dt,
                                          CameraZ);
            WorldMode->DebugLightP = CameraEntity->GroundP + V3(0, 0, 0.75f);
        }
        
        PushLight(RenderGroup, WorldMode->DebugLightP, V3(0.5f, 0.5f, 0.5f), V3(1, 1, 1));
    ...
    EndLightingComputation(&WorldMode->TestLighting, RenderGroup, GameState->HighPriorityQueue);
