...
RayIntersectsBox(v3 RayOrigin, v3 RayD, v3 BoxP, v3 BoxRadius)
{
    ...
    v3 tBoxMin = Hadamard((BoxMin - RayOrigin), InvRayD);
    v3 tBoxMax = Hadamard((BoxMax - RayOrigin), InvRayD);
        ...
        Advance(&Iter))
    {
        ...
        if(Entity->Flags & EntityFlag_Active)
        {
            ...
            if(BoostTo)
            {
                    ...
                    ++TraversableIndex)
                {
                    ...
                    if(Occupier && (Occupier->MovementMode == MovementMode_Planted))
                    {
                        ...
                        if(TransactionalOccupy(SimRegion, Occupier,
                                               &Occupier->Occupying,
                                               Entity->AutoBoostTo))
                        {
            ...
            if(RenderGroup)
            {
                ...
                // TODO(casey): Rewrite tag matching to be based on sorted arrays
                asset_vector MatchVector = {};
                MatchVector.E[Tag_FacingDirection] = Entity->FacingDirection;
                MatchVector.E[Tag_Idle] = 1.0f;
                asset_vector WeightVector = {};
                WeightVector.E[Tag_FacingDirection] = 1.0f;
                WeightVector.E[Tag_Idle] = 1.0f;
                
                for(u32 MatchIndex = 0;
                    MatchIndex < Entity->TagCount;
                    ++MatchIndex)
                {
                    asset_tag_id ID = Entity->Tags[MatchIndex];
                    MatchVector.E[ID] = Entity->TagValue[MatchIndex];
                    WeightVector.E[ID] = 1.0f;
                    if(ID == Tag_Grass)
                    {
                        WeightVector.E[ID] = 1.1f;
                    }
                }
                    ...
                    ++PieceIndex)
                {
                    ...
                    bitmap_id BitmapID =
                        GetBestMatchBitmapFrom(Assets, Piece->AssetCategory, &MatchVector, &WeightVector);
                    ...
                    ++TraversableIndex)
                {
                    ...
                    PushCube(RenderGroup, RenderGroup->WhiteTexture,
                             EntityGroundP + Traversable->P, V3(0.04f, 0.04f, 0.04f),
                             V4(1, 0, 1, 1),
                             DefaultCubeUVLayout(), 0, 0, 3.0f);
                }
#endif
            }
        }
        ...
        ++CoverSpecIndex)
    {
        ...
        if(Spec->Type)
        {
            ...
            switch(Spec->Type)
            {
                ...
                {
                    WeightVector.E[Tag_Stone] = 1.0f;
                    WeightVector.E[Tag_Cover] = 1.0f;
                    MatchVector.E[Tag_Stone] = 1.0f;
                    MatchVector.E[Tag_Cover] = 1.0f;
                } break;
                
                case CoverType_Flowers:
                {
                    WeightVector.E[Tag_Flowers] = 1.0f;
                    MatchVector.E[Tag_Flowers] = 1.0f;
                } break;
                
                case CoverType_ThickGrass:
                {
                    WeightVector.E[Tag_Cover] = 1.0f;
                    WeightVector.E[Tag_Size] = 1.0f;
                    MatchVector.E[Tag_Cover] = 1.0f;
                    MatchVector.E[Tag_Size] = 1.0f;
                } break;
                ...
                case CoverType_SplotchGrass:
                {
                    ...
                    WeightVector.E[Tag_Size] = 1.0f;
                    MatchVector.E[Tag_Cover] = 1.0f;
                    MatchVector.E[Tag_Size] = 0.5f;
                } break;
            }
            WeightVector.E[Tag_Variant] = 1.0f;
                ...
                ++DensityIndex)
            {
                ...
                MatchVector.E[Tag_Variant] = RandomUnilateral(&CoverSeries);
                
                v3 Color = {1, 1, 1};
                
                Cover->Bitmap = GetBestMatchBitmapFrom(Assets, Asset_Particle, &MatchVector, &WeightVector);;
