...
void render_scene(Scene *scene, Framebuffer *framebuffer, const Texture *texture) {
    ...
    clipped_vertices[1] = (v4){0, 3, 15, 1};
    clipped_vertices[2] = (v4){3, 3, 15, 1};
    
    clipped_vertices[3] = (v4){3, 3, 15, 1};
    clipped_vertices[4] = (v4){3, 0, 15, 1};
    ...
    for (int vertex_index = 0; vertex_index < num_clipped_vertices; vertex_index += 3) {
        ...
        v2 vertex0 = get_v2_from_v4(clipped_vertices[vertex_index + 2]);
        v2 vertex1 = get_v2_from_v4(clipped_vertices[vertex_index + 1]);
        v2 vertex2 = get_v2_from_v4(clipped_vertices[vertex_index + 0]);
        v2 viewport_vertices[] = { vertex0, vertex1, vertex2 };
        
        s32 bounding_box_min_x = fmin(fmin(vertex0.x, vertex1.x), vertex2.x);
        s32 bounding_box_max_x = fmax(fmax(vertex0.x, vertex1.x), vertex2.x);
        s32 bounding_box_min_y = fmin(fmin(vertex0.y, vertex1.y), vertex2.y);
        s32 bounding_box_max_y = fmax(fmax(vertex0.y, vertex1.y), vertex2.y);
        
        //TODO: some vertices may be clipped, change this code once clipping is done
        m128i edge_equation_results[3];
        m128i step_x[3];
        m128i step_y[3];
        
        v2i p_min = { bounding_box_min_x, bounding_box_min_y };
        for (int triangle_vertex_index = 0; triangle_vertex_index < 3; ++triangle_vertex_index) {
            v2i vec0 = get_v2i_from_v2(viewport_vertices[triangle_vertex_index]) ;
            v2i vec1 = get_v2i_from_v2(viewport_vertices[(triangle_vertex_index + 1) % 3]);
            
            s32 A = vec0.y - vec1.y;
            s32 B = vec1.x - vec0.x;
            s32 C = vec0.x * vec1.y - vec0.y * vec1.x;
            
            m128i A_m128i = set_broadcast_m128i(A);
            m128i x_m128i = add_m128i(set_broadcast_m128i(p_min.x), set_m128i(0, 1 ,2, 3));
            m128i term1 = _mm_mullo_epi32(x_m128i, A_m128i);
            m128i B_m128i = set_broadcast_m128i(B);
            m128i y_m128i = set_broadcast_m128i(p_min.y);
            m128i term2 = _mm_mullo_epi32(B_m128i, y_m128i);
            m128i term3 = set_broadcast_m128i(C);
            
            edge_equation_results[triangle_vertex_index] = add_m128i(add_m128i(term1, term2), term3);
            
            step_x[triangle_vertex_index] = _mm_mullo_epi32(A_m128i, set_broadcast_m128i(4));
            step_y[triangle_vertex_index] =  B_m128i;
        }
        
        m128i w0_row = edge_equation_results[1];
        m128i w1_row = edge_equation_results[2];
        m128i w2_row = edge_equation_results[0];
...
#if 0
        m128 color0_red = div_m128(set_broadcast_m128(vertex_colors[vertex_index + 2].r), z0_new);
        m128 color1_red =  div_m128(set_broadcast_m128(vertex_colors[vertex_index + 1].r), z0_new);
        m128 color2_red =  div_m128(set_broadcast_m128(vertex_colors[vertex_index + 0].r), z0_new);
        
        m128 color0_green = div_m128(set_broadcast_m128(vertex_colors[vertex_index + 2].g), z1_new);
        m128 color1_green =  div_m128(set_broadcast_m128(vertex_colors[vertex_index + 1].g), z1_new);
        m128 color2_green =  div_m128(set_broadcast_m128(vertex_colors[vertex_index + 0].g), z1_new);
        
        m128 color0_blue = div_m128(set_broadcast_m128(vertex_colors[vertex_index + 2].b), z2_new);
        m128 color1_blue =  div_m128(set_broadcast_m128(vertex_colors[vertex_index + 1].b), z2_new);
        m128 color2_blue =  div_m128(set_broadcast_m128(vertex_colors[vertex_index + 0].b), z2_new);
#endif
        ...
        for (int y = bounding_box_min_y; y <= bounding_box_max_y; ++y) {
            ...
            for (int x = bounding_box_min_x; x <= bounding_box_max_x; x += 4) {
                ...
                if(_mm_test_all_zeros(mask, mask))
                {
                    ...
                    w0 = add_m128i(w0, step_x[1]);
                    w1 = add_m128i(w1, step_x[2]);
                    w2 = add_m128i(w2, step_x[0]);
                    continue;
                }
                ...
                //interpolate color
#if 0
                m128 color_red_nominator = add_m128(add_m128(mul_m128(w0_m128, color0_red), mul_m128(w1_m128, color1_red)), mul_m128(w2_m128, color2_red));
                m128 color_red = div_m128(color_red_nominator, denominator);
                
                m128 color_green_nominator = add_m128(add_m128(mul_m128(w0_m128, color0_green), mul_m128(w1_m128, color1_green)), mul_m128(w2_m128, color2_green));
                m128 color_green = div_m128(color_green_nominator, denominator);
                
                m128 color_blue_nominator = add_m128(add_m128(mul_m128(w0_m128, color0_blue), mul_m128(w1_m128, color1_blue)), mul_m128(w2_m128, color2_blue));
                m128 color_blue = div_m128(color_blue_nominator, denominator);
                
                color_red = mul_m128(color_red, z_interpolation);
                color_green = mul_m128(color_green, z_interpolation);
                color_blue = mul_m128(color_blue, z_interpolation);
                
                m128i color_red_m128i = convert_to_m128i(mul_m128(color_red, set_broadcast_m128(255.0f)));
                m128i color_blue_m128i = convert_to_m128i(mul_m128(color_blue, set_broadcast_m128(255.0f)));
                m128i color_green_m128i = convert_to_m128i(mul_m128(color_green, set_broadcast_m128(255.0f)));
                
                u32 colors[4];
                for (int i = 0; i < 4; ++i) {
                    u32 red = ((u32 *)&color_red_m128i)[i];
                    u32 green = ((u32 *)&color_green_m128i)[i];
                    u32 blue = ((u32 *)&color_blue_m128i)[i];
                    
                    //colors[i] = red << 24 | green << 16 | blue << 8 | 0xFF;
                    colors[i] = 0xFF << 24 | blue << 16 | green << 8 | red;
                }
#endif
                ...
                //TODO: this math part is sketchy as fuck, check it again... this is just a termporary patch
                ...
                //draw pixels
#if 0
                m128i output_pixels = set_m128i(colors[0], colors[1], colors[2], colors[3]);
#else
            ...
            w0_row = add_m128i(w0_row, step_y[1]);
            w1_row = add_m128i(w1_row, step_y[2]);
            w2_row = add_m128i(w2_row, step_y[0]);
        }
        
        u64 new_current_clock_cycles = __rdtscp(&temp_for_clocks);
        g_rasterizer_clock_cycles = (new_current_clock_cycles - current_clock_cycles) / ((bounding_box_max_x - bounding_box_min_x) * (bounding_box_max_y - bounding_box_min_y));
    }
}
