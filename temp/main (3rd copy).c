...
Texture load_texture(const char *path) {
    ...
    return (Texture){.width = (u32)width, .height = (u32)height, .channels = (u32)channels, .data = data};
}
...
Framebuffer create_framebuffer(u32 width, u32 height) {
    ...
    v4 default_color = get_v4(0.1f, 0.1f, 0.1f, 1.0f);
...
Image *image_create(u32 width, u32 height, u32 channels) {
    ...
    u32 size = (u32)sizeof(unsigned char) * num_elements;
...
void blit_image(Framebuffer *framebuffer, Image *image) {
    ...
    for (u32 y = 0; y < height; y++) {
        for (u32 x = 0; x < width; x++) {
            ...
            u32 dst_index = (flipped_y * width + x) * 4;
...
m4 get_m4_perspective(f32 fovy, f32 aspect, f32 near, f32 far) {
    f32 z_range = far - near;
    m4 m;
    assert(fovy > 0 && aspect > 0);
    assert(near > 0 && far > 0 && z_range > 0);
    f32 elements[4][4] = {
        { 1.0f / (aspect * (float)tan(fovy / 2)), 0, 0, 0 },
        { 0, 1.0f / (float)tan(fovy / 2), 0, 0 },
        { 0, 0, -1.0f * (far + near) / (far - near),  -2 * (far * near) / (far - near)} ,
        { 0, 0, -1.0f, 0 }
    };
    
    memcpy(m.e, elements,  4 * 4 * sizeof(f32));
...
Scene create_scene(u32 width, u32 height) {
    Scene scene = {0};
    scene.camera = create_camera(get_v3(0.0f, 0.0f, 1.5f), get_v3(0, 0, 0), (f32)width / (f32)height);
...
void render_scene(Scene *scene, Framebuffer *framebuffer, const Texture *texture) {
    ...
    const u32 stride = 8;
    const u32  vertices_num = array_count(input_vertices) / 8;
    //v4 vertex_positions[6];
    //v4 vertex_colors[6];
    v4 *vertex_positions = (v4 *)malloc((u32)sizeof(v4) * vertices_num);
    v4 *vertex_colors = (v4 *)malloc((u32)sizeof(v4) * vertices_num);
    v2 *vertex_uvs = (v2 *)malloc((u32)sizeof(v2) * vertices_num);
    ...
    for (u32 i = 0; i < num_clipped_vertices; ++i) {
    ...
    for (u32 i = 0; i  < num_clipped_vertices; ++i) {
        ...
        vertex->y = (f32)framebuffer->height - (vertex->y  + 1) * 0.5f * (float)framebuffer->height;
    ...
    clipped_vertices[3] = (v4){255, 255, 15, 1};
    clipped_vertices[4] = (v4){255, 0, 15, 1};
    clipped_vertices[5] = (v4){0, 0, 15, 1};
#endif
    
    for (u32 vertex_index = 0; vertex_index < num_clipped_vertices; vertex_index += 3) {
        ...
        u32 start_x = (u32)min_s32(min_s32(vertex0.x, vertex1.x), vertex2.x);
        u32 end_x = (u32)max_s32(max_s32(vertex0.x, vertex1.x), vertex2.x);
        u32 start_y = (u32)min_s32(min_s32(vertex0.y, vertex1.y), vertex2.y);
        u32 end_y = (u32)max_s32(max_s32(vertex0.y, vertex1.y), vertex2.y);
        ...
        m128 z0_reciprocal = set_broadcast_m128(1.0f / clipped_vertices[vertex_index + 2].z);
        m128 z1_reciprocal = set_broadcast_m128(1.0f / clipped_vertices[vertex_index + 1].z);
        m128 z2_reciprocal = set_broadcast_m128(1.0f / clipped_vertices[vertex_index + 0].z);
        
        for (u32 y = start_y; y <= end_y; y += y_increment) {
            m128i w0 = w0_row;
            m128i w1 = w1_row;
            m128i w2 = w2_row;
            
            for (u32 x = start_x; x <= end_x; x += x_increment) {
                ...
                m128 u_texture_value_temp = mul_m128(interpolated_u, set_broadcast_m128((f32)(texture->width - 1)));
                m128 v_texture_value_temp = mul_m128(interpolated_v, set_broadcast_m128((f32)(texture->height - 1)));
                
                m128i u_texture_value = convert_to_m128i(u_texture_value_temp);
                m128i v_texture_value = convert_to_m128i(v_texture_value_temp);
                
                m128i texture_offsets = add_m128i(mul_m128i(v_texture_value, set_broadcast_m128i((s32)texture->width)), u_texture_value);
                texture_offsets = mul_m128i(texture_offsets, set_broadcast_m128i((s32)texture->channels));
                
                u32 texture_pixels[4] = {0};
                for (int i = 0; i < 4; ++i) {
                    u32 texture_index = (u32)((s32 *)&texture_offsets)[i];
                ...
                //draw pixels
                m128i output_pixels_ = set_m128i((s32)texture_pixels[0], (s32)texture_pixels[1], (s32)texture_pixels[2], (s32)texture_pixels[3]);
                //m128i output_pixels_ = set_m128i(0xFF0000FF, 0xFF0000FF, 0xFF0000FF, 0xFF0000FF);
...
#else
                u32 *mask_s32 = (u32 *)&mask;
                int *output_pixels = (int *)&output_pixels_;
                
                if (mask_s32[0] == 0xFFFFFFFF) {
                    ((int *)framebuffer->color_buffer)[(y * framebuffer->width + x)] = output_pixels[0];
                }
                if (mask_s32[1] == 0xFFFFFFFF) {
                    ((int *)framebuffer->color_buffer)[(y * framebuffer->width + x + 1)] = output_pixels[1];
                }
                if (mask_s32[2] == 0xFFFFFFFF) {
                    ((int *)framebuffer->color_buffer)[(y * framebuffer->width + framebuffer->width + x)] = output_pixels[2];
                }
                if (mask_s32[3] == 0xFFFFFFFF) {
                    ((int *)framebuffer->color_buffer)[(y * framebuffer->width  + framebuffer->width + x + 1)] = output_pixels[3];
                }
#endif
                w0 = add_m128i(w0, A0_increment);
                w1 = add_m128i(w1, A1_increment);
...
int main() {
    ...
    size_hints->min_width = (s32)screen_width;
    size_hints->max_width = (s32)screen_width;
    size_hints->min_height = (s32)screen_height;
    size_hints->max_height = (s32)screen_height;
    ...
    //create surface
    u32 depth = (u32)XDefaultDepth(display, screen);
    ...
    printf("lowest fps: %f, highest_fps: %f, average fps: %f\n", lowest_fps, highest_fps, 1.0f / (average_fps / (f32)average_fps_counter));
    assert(display != NULL);
