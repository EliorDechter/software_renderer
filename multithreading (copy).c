...
#define MAX_CONTINUATIONS_NUM 20
...
Job *create_job(Job_group *job_group, Job_function function, void *data, Job *parent) {
    Job job = {0};
    job.function = function;
    job.unfinished_jobs = 1;
    job.data = data;
    job.jobs_to_delete_count = 0;
    job.id = 0; //TODO: add thread id or name
    assert(job_group->job_count + 1 < MAX_JOB_NUM);
    job_group->jobs[job_group->job_count++] = job;
    
    return &job_group->jobs[job_group->job_count - 1];
}
...
Job *create_root_job(Job_group *job_group, Job_function function, void *data) {
    ...
    return create_job(job_group, function, data, NULL);
}

Job *create_child_job(Job_group *job_group, Job_function function, void *data, Job *parent) {
    ...
    return create_job(job_group, function, data, parent);
}
...
void parallel_for_function(Worker *worker, Job *parent_job) {
    ...
    if (parallel_for_data->array_count <= parallel_for_data->grain_size) {
        (parallel_for_data->function)(parallel_for_data->begin_index, parallel_for_data->end_index, parallel_for_data->array, parallel_for_data->data);
    }
    else {
        ...
        Job *job0 = create_child_job(job_group, parallel_for_function, first_half_parallel_for_data, parent_job);
        Job *job1 = create_child_job(job_group, parallel_for_function, second_half_parallel_for_data, parent_job);
